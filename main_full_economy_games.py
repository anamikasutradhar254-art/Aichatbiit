...
async def rob_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    amount = parse_amount(context.args[0]) if context.args else None
    if not amount or amount <= 0:
        await update.message.reply_text("Usage: /rob <amount> reply karke")
        return
...
async def kill_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    amount = parse_amount(context.args[0]) if context.args else None
    if not amount or amount <= 0:
        await update.message.reply_text("Usage: /kill <amount> reply karke")
        return
...
/rob <amount> - Reply and rob user
/kill <amount> - Reply and attack user
...
11. Time 45 sec se start hoga, har round 2 sec kam hoga, minimum 10 sec.
...
async def round_timeout(chat_id, player_id, round_no, context: ContextTypes.DEFAULT_TYPE):
    ...
    await asyncio.sleep(game.get("time_limit", 45))
