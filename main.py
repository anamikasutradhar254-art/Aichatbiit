...
1. /connectwin bhejo.
2. Bot 30 seconds join time dega.
3. Players /join bhej kar game me aayenge.
4. Minimum 2 players chahiye.
...
async def connectwin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.id in connect_games:
        await update.message.reply_text("⚠️ Game already chal raha hai.")
        return
    connect_games[chat.id] = {"joining": True, "active": False, "players": {}, "score": {}, "level": 0, "current_answer": None, "used": []}
    await update.message.reply_text("🔗 ᴄᴏɴɴᴇᴄᴛᴡɪɴ ʙᴀᴛᴛʟᴇ ꜱᴛᴀʀᴛᴇᴅ!\n\nJoin karne ke liye /join bhejo.\n⏳ Join time: 30 seconds\n👥 Minimum players: 2")
    ...
    if len(game["players"]) < 2:
        ...
        await context.bot.send_message(chat.id, "❌ Game cancel ho gaya. Minimum 2 players chahiye.")
        return
    game["joining"] = False
    game["active"] = True
    await context.bot.send_message(chat.id, f"✅ Game started!\nPlayers: {len(game['players'])}\n\n10 levels battle ready 🔥")
...
async def join_connect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ {mention_user(user)} joined ConnectWin!", parse_mode=ParseMode.HTML)
...
async def ai_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are Ada. Sabse friendly tareeke se baat karo. Hindi me baat karo lekin English alphabets me. Short aur natural replies do. User jaise baat kare waise reply do. Human friend ki tarah behave karo."},
