...
COUPLE_PHOTOS = [
    "https://i.ibb.co/2kR5zqf/anime-couple.jpg",
    "https://i.ibb.co/X2B2c2t/couple-anime.jpg"
]
...
def get_name(user):
    return html.unescape(user.first_name or "Unknown")
...
def track_active(update: Update):
    ...
    active_members[chat_id].append(member)
...
async def crush(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"💘 {get_name(user)}'s Sᴇᴄʀᴇᴛ Cʀᴜꜱʜ Iꜱ - {crush_user['name']}\n\n"
...
async def brain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🧠 IQ Lᴇᴠᴇʟ Oꜰ {get_name(user)} Iꜱ {iq}% 😎"
    )
...
async def action_command(update: Update, context: ContextTypes.DEFAULT_TYPE, action):
    ...
    sender = get_name(update.effective_user)
    target = get_name(update.message.reply_to_message.from_user)
