...
async def connectwin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.id in connect_games:
        await update.message.reply_text("⚠️ Game already chal raha hai. End karne ke liye /endconnectwin use karo.")
        return
    ...
    await update.message.reply_text(
        "🔗 ᴄᴏɴɴᴇᴄᴛᴡɪɴ ʙᴀᴛᴛʟᴇ ꜱᴛᴀʀᴛᴇᴅ!\n\n"
        "Join karne ke liye /join bhejo.\n"
        "⏳ Join time: 30 seconds\n"
        "👥 Minimum players: 2"
    )

    context.application.create_task(start_connect_after_delay(chat.id, context))


async def start_connect_after_delay(chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game["players"]) < 2:
        ...
        await context.bot.send_message(chat_id, "❌ Game cancel ho gaya. Minimum 2 players chahiye.")
        return

    game["joining"] = False
    game["active"] = True

    await context.bot.send_message(chat_id, f"✅ Game started!\nPlayers: {len(game['players'])}\n\n10 levels battle ready 🔥")
    await send_next_connect_level(chat_id, context)
