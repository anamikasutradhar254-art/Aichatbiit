...
Correct +1 | Run out -1
Time 20 sec se start, minimum 10 sec
...
9. 20 sec me answer nahi diya toh run out, -1 point aur game se bahar.
10. Har round me time 2 sec kam hoga: 20, 18, 16, 14, 12, 10.
...
async def startchaingame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Time 20 sec se start hoga, har round 2 sec kam hoga, minimum 10 sec."
    )
...
async def send_next_opposite_round(chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    game["round"] += 1
    game["time_limit"] = max(10, 20 - ((game["round"] - 1) * 2))
    game["answered"] = False
